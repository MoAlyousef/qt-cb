...
macro_rules! impl_ext {
    ($name: ident) => {
        impl TextExt for QBox<$name> {
            unsafe fn on_redo_available<F: FnMut(&Self, bool) + 'static>(&self, mut cb: F) {
                utils::connect_1a!(QTextEdit, self, redo_available, SlotOfBool, cb);
            }
            unsafe fn on_selection_changed<F: FnMut(&Self) + 'static>(&self, mut cb: F) {
                utils::connect_0a!(QTextEdit, self, selection_changed, cb);
            }
            unsafe fn on_text_changed<F: FnMut(&Self) + 'static>(&self, mut cb: F) {
                utils::connect_0a!(QTextEdit, self, text_changed, cb);
            }
            unsafe fn on_undo_available<F: FnMut(&Self, bool) + 'static>(&self, mut cb: F) {
